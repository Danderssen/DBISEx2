...
public class MovieService extends MovieServiceBase {
	...
	public DBCursor getGeotaggedTweets(int limit) {
		DBCursor results = tweets.find(new BasicDBObject("coordinates", new BasicDBObject("$exists", true))).limit(limit);
		return results;
	}
	...
	public DBCursor getTaggedTweets() {
		DBObject projection = new BasicDBObject("coordinates", true)
				.append("text", true)
				.append("movie", true)
